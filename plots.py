...
'''
def distancing(people_coords, img, dist_thres_lim=(100,125)):
...
def distancing(people_coords, img, intermediate, high, normal,dist_thres_lim=(100,150)):
    already_red = dict() # dictionary to store if a plotted rectangle has already been labelled as high risk
    centers = []
    for i in people_coords:
        centers.append(((int(i[2])+int(i[0]))//2,(int(i[3])+int(i[1]))//2))
    ...
    for x in x_combs:
        ...
        cntr1 = ((int(xyxy1[2])+int(xyxy1[0]))//2,(int(xyxy1[3])+int(xyxy1[1]))//2)
        cntr2 = ((int(xyxy2[2])+int(xyxy2[0]))//2,(int(xyxy2[3])+int(xyxy2[1]))//2)
        dist = (((cntr2[0]-cntr1[0])**2 + (cntr2[1]-cntr1[1])**2)**0.5)/1.28
        ...
        if dist > dist_thres_lim[0] and dist < dist_thres_lim[1]:
            ...
            m = round(dist)/100
            cv2.putText(img, str(m)+" m", mid, cv2.FONT_HERSHEY_SIMPLEX , 1, color, 2, cv2.LINE_AA)
            already_red[cntr1] = 0
            already_red[cntr2] = 0
            cv2.circle(img, cntr1, radius, color, -1)
            cv2.circle(img, cntr2, radius, color, -1)
            ...
            for xy in x:
                cntr = ((int(xy[2])+int(xy[0]))//2,(int(xy[3])+int(xy[1]))//2)
                if already_red[cntr] == 0:
                    z1, z2 =(int(xyxy1[0]),int(xyxy1[1])),(int(xyxy1[2]),int(xyxy1[3]))
                    zz1, zz2 =(int(xyxy2[0]),int(xyxy2[1])),(int(xyxy2[2]),int(xyxy2[3]))
                    cv2.rectangle(img, z1, z2, color, thickness=tl, lineType=cv2.LINE_AA)
                    cv2.rectangle(img, zz1, zz2, color, thickness=tl, lineType=cv2.LINE_AA)
    ...
    for count in already_red:
        if already_red[count] == 0:
            intermediate += 1
        if already_red[count] == 1:
            high += 1
        if already_red[count] == 2:
            normal += 1
    
    return(normal,intermediate,high)
